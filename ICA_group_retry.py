...
import pandas as pd
import mne
from our_group_ICA import  plotCumulativeExplainedVariances, pvaf, componentPlot,timeSeriesPlot, timeSeriesPlotICA, loadData, common, montage, componentTimeseriesPlot, componentTimeseriesPlotIndividual, pvaf_new_ICA, center_data
...
for i in range(num_subjects):
    mixtures[i] = mixtures[i] - mixtures[i].mean(axis=1).reshape(36,1)
...
assert np.allclose(X_pca_red, np.dot(estimated_sources, estimated_mixing_matrices.T))
print("Success!")
...
for i in range(num_subjects):
    for j in range(n_components*num_subjects):
        G_i = whitening_matrix.T[i*n_components:(i+1)*n_components, :]
        R_i = R_pca[i]
        A = estimated_mixing_matrices[i*n_components:(i+1)*n_components, :]
        ...
        backproj[i] = np.transpose(np.dot(R_i,A))
