...
mixtures = [np.dot(mixing_matrices[i], sources[i]) for i in range(num_subjects)]
...
for i in range(num_subjects):
    pca = PCA(n_components=num_sources, whiten=False)
    S_pca[i] = pca.fit_transform(mixtures[i].T)
    A_pca[i] = pca.components_  # Get estimated mixing matrix
    X_pca[i] = np.dot(S_pca[i], A_pca[i].T)  # Estimate the sources
...
for i in range(num_plots):
    ...
    for j in range(num_sources):
        plt.plot(sources[i][j], label=f"Subject {i+1}, Source {j+1}")
    plt.legend()
