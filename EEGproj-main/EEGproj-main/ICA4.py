...
for filename in Speech_files:
    ...
    path = path.replace(" ","")
    raw = mne.io.read_epochs_eeglab(path, montage_units='dm',) # added 'preload=True'
    events = mne.find_events(raw, stim_channel=common)
    event_id = ['Tagi_A','Tabi_A']
    epochs = mne.Epochs(raw, events, event_id, tmin=-0.5, tmax=1.5, baseline=(None, 0), reject=dict(eeg=100e-6))
